...
- 键也可以用来直接跳过一步，而不只是还原它的输入：一个单元测试 job，如果命令、镜像摘要和源码树的哈希与之前某次运行相同，就可以直接报告那次的结果——于是键里漏掉的任何东西（一个系统包、一次网络响应）不再只是多编译一遍，而是变成一次错误的通过。缓存放在哪里是另一处取舍：放在节点自己的磁盘上，命中不花钱，但只服务落到这台节点上的 job，所以调度器可以优先（而不是强制）挑一台最近跑过同一个仓库的节点；放在对象存储里每台节点都能命中，但要靠一次下载来还原，130 秒的 job 未必赚得回来。
