...
先向面试官确认 Part 3 具体指哪一种规则：选 `calculate_damage` 对当前防御者算出的伤害最高的那个（下面用的规则），
还是不看伤害数值、只选属性克制防御者的那个——当一个 `1x` 攻击者的攻击力高到能压过 `2x` 攻击者时，两种规则会给出不同答案，
例如攻击力 `10`、`1x`（伤害 `10`）对上攻击力 `3`、`2x`（伤害 `6`）：伤害最高规则选前者，属性克制规则选后者。
